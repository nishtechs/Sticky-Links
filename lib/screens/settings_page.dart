...
class SettingsPage extends StatelessWidget {
      ...
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Failed to parse import file format.'),
            ),
          );
        }
      }
  ...
  Future<void> _importBookmarks(BuildContext context) async {
    ...
    if (result != null) {
      ...
      final content = await file.readAsString();
      try {
        final List<LinkItem> importedLinks = BookmarkService.parseBookmarkHtml(
          content,
        );
        if (!context.mounted) return;
        final linksProvider = Provider.of<LinksProvider>(
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Import complete! Added $added new links from bookmarks.',
              ),
            ),
          );
        }
    ...
    if (!hasPermission) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Storage permission required to change backup folder.',
            ),
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isDesktop
          ? _buildSettingsList(context, colorScheme, settings)
          : SafeArea(child: _buildSettingsList(context, colorScheme, settings)),
    );
  }
  ...
  Widget _buildSettingsList(
    ...
    return ListView(
      ...
      children: [
        ...
        // Shortcuts
        if (isDesktop) ...[
          _buildSectionHeader('Shortcuts', Icons.keyboard_rounded, colorScheme),
          const SizedBox(height: 12),
          _buildSettingsCard(
            colorScheme: colorScheme,
            children: [
    ...
    return ListTile(
      ...
      trailing: Container(
        ...
        child: Text(
          shortcut,
          style: TextStyle(
            fontFamily: Platform.isMacOS ? 'Monaco' : 'Consolas',
